...
async function writeFile (inFileName, inContent) {
  ...
  return new Promise((resolve, reject) => {
    fs.writeFile(inFileName, inContent, 'utf8', function (err) {
      if (err) {
        console.warn('\x1b[31mAn error occured while writing to file.\x1b[0m')
        reject(err)
      } else {
...
/**
   ...
   * @returns {Promise} - Resolves if delete succesfull
   */
...
/**
 * Evaluate a fair number to devide the input into, for debugging loops
 * @param {number} amount
